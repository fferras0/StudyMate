...
import { Sparkles, Brain, FileText, Loader2, X, Settings2, Languages, HelpCircle, ArrowRight, ArrowLeft, Edit3, Save, RotateCcw, Timer, Clock, Trash2, Image as ImageIcon, Type, ALargeSmall, Lock, LogOut, History } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen bg-gray-50 flex flex-col ${isRTL ? 'font-[Noto_Sans_Arabic]' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
      {/* Header */}
      <header className="bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-20 print:hidden transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-3 text-indigo-600 cursor-pointer" onClick={handleBackToMenu}>
            <div className="bg-indigo-600 text-white p-1.5 rounded-lg shadow-md shadow-indigo-200">
               <Brain className={`w-6 h-6 ${isRTL ? 'ml-0' : ''}`} />
            </div>
            <h1 className="text-xl font-bold tracking-tight text-slate-800 hidden md:block">{t.title}</h1>
          </div>
          <div className="flex items-center gap-3">
             {/* User Info */}
             <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-full text-sm font-bold">
                {user.isGuest ? (
                   <>
                     <span className="text-xs uppercase tracking-wider bg-indigo-200 px-1.5 py-0.5 rounded">GUEST</span>
             ...
             <button 
                ...
                className="px-3 py-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 font-medium text-sm flex items-center gap-2 transition-colors"
                title={t.history}
             >
                <History size={16} />
             ...
             {currentFile && appState !== AppState.IDLE && appState !== AppState.EDIT_TEXT_VIEW && (
                <button 
                  onClick={handleReset} 
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-red-50 text-red-600 hover:bg-red-100 font-medium text-sm transition-colors"
                >
                  <Trash2 size={14} />
        ...
        <div className="max-w-7xl mx-auto print:max-w-none">
          ...
          {currentFile && appState === AppState.IDLE && (
            <div className="max-w-5xl mx-auto animate-in slide-in-from-bottom-8 duration-500">
               ...
               {/* Improved File Card */}
               <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden mb-8 transform transition-all hover:scale-[1.01]">
                 <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-5">
                      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${currentFile.mimeType.includes('image') ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'}`}>
                        {currentFile.mimeType.includes('image') ? <ImageIcon size={32} /> : <FileText size={32} />}
                      </div>
                      <div className="overflow-hidden">
                        <div className="flex items-center gap-2 mb-1">
                             <span className="text-xs font-bold px-2 py-0.5 rounded bg-slate-100 text-slate-500 uppercase tracking-wide">
                                {isUsingEditedText ? 'TEXT' : currentFile.mimeType.split('/')[1]}
                             </span>
                        </div>
                        <h3 className="font-bold text-slate-900 truncate max-w-[250px] md:max-w-md text-xl md:text-2xl">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                         {isUsingEditedText ? (
                            <button 
                                ...
                                className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 text-sm font-bold text-amber-700 bg-amber-50 rounded-xl hover:bg-amber-100 transition-colors border border-amber-100"
                            >
                                <RotateCcw size={16} />
                                {t.revertOriginal}
                            </button>
                            ...
                            <button 
                                ...
                                className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 text-sm font-bold text-indigo-700 bg-indigo-50 rounded-xl hover:bg-indigo-100 transition-colors border border-indigo-100"
                            >
                                <Edit3 size={16} />
                                {t.editText}
                            </button>
                         )}
                        <button onClick={handleReset} className="p-3 text-slate-400 hover:text-red-500 rounded-xl hover:bg-red-50 transition-colors border border-transparent hover:border-red-100">
                             <X size={20} />
                        </button>
                    </div>
                 </div>
                   ...
                   <div className="grid grid-cols-2 gap-4 mb-6 animate-in slide-in-from-top-4">
                       {summary && (
                           <button 
                               ...
                               className="py-3 px-4 bg-indigo-100 text-indigo-700 font-bold rounded-xl hover:bg-indigo-200 transition-colors flex items-center justify-center gap-2"
                           >
                               <FileText size={20} />
                               {t.viewSummary}
                           </button>
                       ...
                       {quiz.length > 0 && (
                           <button 
                               ...
                               className="py-3 px-4 bg-emerald-100 text-emerald-700 font-bold rounded-xl hover:bg-emerald-200 transition-colors flex items-center justify-center gap-2"
                           >
                               <Brain size={20} />
                               {t.startQuizAgain}
                           </button>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  {/* Settings Panel */}
                  <div className="lg:col-span-4 bg-white rounded-3xl shadow-lg border border-slate-100 p-8 h-fit">
                    <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                      <div className="p-2 bg-slate-100 rounded-lg text-slate-600">
                         <Settings2 size={20} />
                      </div>
                      <h3 className="font-bold text-xl text-slate-800">{t.settings}</h3>
                    </div>
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-3 p-1.5 bg-slate-100 rounded-2xl">
                          <button 
                            ...
                            className={`py-2.5 px-4 text-sm font-bold rounded-xl transition-all ${language === 'ar' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                          >
                            العربية
                          </button>
                          <button 
                            onClick={() => setLanguage('en')}
                            className={`py-2.5 px-4 text-sm font-bold rounded-xl transition-all ${language === 'en' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                          >
                            English
                          </button>
                        </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                           ...
                           <div className="grid grid-cols-3 gap-2">
                              {['small', 'medium', 'large'].map((size) => (
                                <button
                                  ...
                                  className={`py-2 px-1 text-xs font-bold rounded-xl border transition-all ${fontSize === size ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'}`}
                                >
                           ...
                           {/* Font Type */}
                           <div className="grid grid-cols-3 gap-2">
                             <button onClick={() => setFontType('sans')} className={`py-2 rounded-xl text-xs font-bold border ${fontType === 'sans' ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'bg-white border-slate-200 text-slate-500'}`}>{t.sans}</button>
                             <button onClick={() => setFontType('serif')} className={`py-2 rounded-xl text-xs font-bold border font-serif ${fontType === 'serif' ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'bg-white border-slate-200 text-slate-500'}`}>{t.serif}</button>
                             <button onClick={() => setFontType('mono')} className={`py-2 rounded-xl text-xs font-bold border font-mono ${fontType === 'mono' ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'bg-white border-slate-200 text-slate-500'}`}>{t.mono}</button>
                           </div>

                           {/* Live Font Preview */}
                           <div className={`mt-4 p-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 transition-all ${getFontPreviewClass()}`}>
                              <span className="text-xs font-bold text-slate-400 uppercase mb-1 block">{t.fontPreview}</span>
                              {t.sampleText}
                           </div>
                        </div>
                      </div>
                  ...
                  <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-5">
                    <button 
                        ...
                        className="group relative p-8 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-3xl shadow-xl shadow-indigo-200 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl overflow-hidden flex flex-col justify-between h-64 border-0"
                      >
                    ...
                    <button 
                        ...
                        className="group relative p-8 bg-white rounded-3xl shadow-xl shadow-slate-200 border border-slate-100 transition-all duration-300 hover:scale-[1.02] hover:border-emerald-200 hover:shadow-emerald-100 overflow-hidden flex flex-col justify-between h-64"
                      >
                        <div className={`absolute top-0 p-8 opacity-[0.03] ${isRTL ? 'left-0' : 'right-0'}`}>
                          <Brain size={120} className="text-emerald-900 rotate-12" />
                        </div>
                        <div className="relative z-10 w-14 h-14 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-600 mb-auto">
                            <Brain size={28} />
                        </div>
                        <div className="relative z-10">
                          <h3 className="text-2xl font-bold mb-2 text-slate-800 group-hover:text-emerald-700 transition-colors">{t.quizBtn}</h3>
          ...
          {appState === AppState.EDIT_TEXT_VIEW && (
              <div className="max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-8">
                  <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 flex flex-col h-[80vh] overflow-hidden">
                      <div className="p-5 px-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/80 backdrop-blur-md">
                          <div className="flex items-center gap-3 text-indigo-700">
                             <div className="p-2 bg-indigo-100 rounded-lg"><Edit3 size={20} /></div>
                      ...
                      <div className="p-4 bg-indigo-50/50 text-sm text-indigo-900 border-b border-indigo-100 flex items-center justify-center">
                         <span className="font-medium">{t.editDesc}</span>
                      </div>

                      <div className="flex-grow relative">
                         <textarea 
                            className="w-full h-full p-8 focus:outline-none resize-none text-slate-800 leading-relaxed text-lg font-mono bg-slate-50"
                            value={extractedText}
                      ...
                      <div className="p-5 px-8 border-t border-slate-100 flex justify-end gap-4 bg-white">
                          <button 
                             onClick={cancelEdit}
                          ...
                          <button 
                             ...
                             className="px-8 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-200 hover:shadow-indigo-300 transition-all flex items-center gap-2 transform hover:-translate-y-0.5"
                          >
                             <Save size={18} />
                             {t.saveText}
                          </button>
          ...
          {appState === AppState.QUIZ_CONFIG && (
            <div className="flex flex-col items-center justify-center min-h-[50vh] max-w-xl mx-auto animate-in fade-in zoom-in-95">
                <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 p-8 md:p-10 w-full relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-emerald-400 to-teal-500"></div>
                    
                    <div className="flex items-center justify-between mb-8">
                        <h3 className="text-3xl font-bold text-slate-800 flex items-center gap-3">
                            <div className="p-2 bg-emerald-100 text-emerald-600 rounded-xl"><Brain size={28} /></div>
                            {t.quizConfigTitle}
                        </h3>
                        <button onClick={handleBackToMenu} className="text-slate-400 hover:text-slate-600 p-2 hover:bg-slate-100 rounded-full transition-colors">
                            <X size={24} />
                        </button>
                    </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div>
                            ...
                            <div className="flex items-center border-2 border-slate-200 rounded-2xl px-5 py-4 focus-within:border-emerald-500 focus-within:ring-4 focus-within:ring-emerald-50 transition-all bg-white">
                                <input 
                                    type="number" 
                                    min="1" 
                                    max="50"
                                    ...
                                    className="w-full bg-transparent outline-none text-2xl font-bold text-slate-900 placeholder-slate-300"
                                />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-3 mb-4">
                                <button 
                                    ...
                                    className={`py-3 px-2 rounded-xl text-sm font-bold border-2 transition-all ${timerMode === 'off' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                                >
                                    {t.timerOff}
                                </button>
                                <button 
                                    onClick={() => { setTimerMode('per_question'); setTimerValue(60); }}
                                    className={`py-3 px-2 rounded-xl text-sm font-bold border-2 transition-all ${timerMode === 'per_question' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                                >
                                    {t.timerPerQ}
                                </button>
                                <button 
                                    onClick={() => { setTimerMode('total'); setTimerValue(10); }}
                                    className={`py-3 px-2 rounded-xl text-sm font-bold border-2 transition-all ${timerMode === 'total' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                                >
                                    {t.timerTotal}
                                </button>
                            </div>
                            ...
                            {timerMode !== 'off' && (
                                <div className="animate-in fade-in slide-in-from-top-2">
                                     <div className="flex items-center gap-3 border-2 border-slate-200 rounded-2xl px-5 py-3 bg-white">
                                        <input 
                                            type="number" 
                                            min="1"
                                            value={timerValue}
                                            onChange={(e) => setTimerValue(Math.max(1, parseInt(e.target.value) || 0))}
                                            className="w-full outline-none text-xl font-bold text-slate-900"
                                        />
                    ...
                    <div className="flex gap-4 mt-10">
                        <button 
                            ...
                            className="flex-1 py-4 px-6 rounded-2xl border-2 border-slate-200 text-slate-600 font-bold hover:bg-slate-50 transition-colors text-lg"
                        >
                            {t.cancel}
                        </button>
                        <button 
                            onClick={startQuizGeneration}
                            className="flex-1 py-4 px-6 rounded-2xl bg-emerald-600 text-white font-bold hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-200 flex items-center justify-center text-lg transform hover:-translate-y-1"
                        >
                            {t.startQuiz}
          ...
          {showLimitModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in">
               <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl transform scale-100 border border-slate-100">
                  <div className="w-16 h-16 bg-red-100 text-red-500 rounded-2xl flex items-center justify-center mb-6 mx-auto">
                     <Lock size={32} />
                  </div>
                  <h3 className="text-2xl font-bold text-center text-slate-800 mb-2">{t.guestLimitTitle}</h3>
          ...
          {appState === AppState.PROCESSING && (
            <div className="flex flex-col items-center justify-center min-h-[50vh]">
              ...
              <h3 className="text-2xl font-bold text-slate-800 mt-8 mb-2">{t.analyzing}</h3>
              <p className="text-slate-500 text-lg">{t.wait}</p>
            </div>
          )}

          {/* Error State */}
          {appState === AppState.ERROR && (
             <div className="max-w-md mx-auto mt-10 p-8 bg-red-50 border border-red-100 rounded-3xl text-center shadow-lg">
               ...
               <button 
                 ...
                 className="px-8 py-3 bg-white text-red-600 border border-red-200 rounded-xl hover:bg-red-100 font-bold transition-colors shadow-sm"
               >
                 {t.tryAgain}
               </button>
             </div>
